...
public class Solution {
	...
	public TreeNode reConstructBinaryTree3(int[] pre, int[] in) {
		...
		TreeNode root = new TreeNode(pre[0]);
		int pivot = 0;
		for (; pivot < in.length; pivot++) {
			if (in[pivot] == pre[0]) {
				break;
			}
		}
		root.left = reConstructBinaryTree3(Arrays.copyOfRange(pre, 1, pivot + 1), Arrays.copyOfRange(in, 0, pivot));
		root.right = reConstructBinaryTree3(Arrays.copyOfRange(pre, pivot + 1, pre.length),
				Arrays.copyOfRange(in, pivot + 1, in.length));
		return root;
	}
